...
from domain_hunter.config.settings import BASE_DIR, CATEGORIES, MONTHS, STATUSES
from domain_hunter.database.db import connect, init_db, query_domains, row_to_dict, stats, upsert_domain
...
def domains_page(request: Request, q: str = "", category: str = "", status: str = "", month: int | None = None, rating_min: int | None = None):
    filters = {k: v for k, v in {"q": q, "category": category, "status": status, "month": month, "rating_min": rating_min}.items() if v not in (None, "")}
    return templates.TemplateResponse("domains.html", {"request": request, "domains": query_domains(filters, limit=500), "filters": filters, "categories": CATEGORIES, "statuses": STATUSES, "months": MONTHS})
...
def calendar_page(request: Request, year: int | None = None, month: int | None = None):
    ...
    return templates.TemplateResponse("calendar.html", {"request": request, "year": year, "month": month, "month_name": MONTHS[month - 1], "by_day": by_day})
...
def domain_detail(request: Request, domain_id: int):
    ...
    return templates.TemplateResponse("domain_detail.html", {"request": request, "domain": row, "history": history})
...
@app.get("/export/{fmt}")
def export_route(fmt: str, category: str = "", status: str = "", month: int | None = None, q: str = ""):
    filters = {k: v for k, v in {"category": category, "status": status, "month": month, "q": q}.items() if v not in (None, "")}
