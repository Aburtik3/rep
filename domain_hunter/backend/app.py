...
def domains_page(request: Request, q: str = "", category: str = "", status: str = "", month: int | None = None, rating_min: int | None = None):
    ...
    return templates.TemplateResponse(
        request=request,
        name="domains.html",
        context={"domains": query_domains(filters, limit=500), "filters": filters, "categories": CATEGORIES, "statuses": STATUSES, "months": MONTHS},
    )
...
def calendar_page(request: Request, year: int | None = None, month: int | None = None):
    ...
    return templates.TemplateResponse(
        request=request,
        name="calendar.html",
        context={"year": year, "month": month, "month_name": MONTHS[month - 1], "by_day": by_day},
    )
